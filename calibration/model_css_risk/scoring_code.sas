...
when ( 2.0091145833 < ACT_CALL_CC ) do;
SCORECARD_POINTS=sum(SCORECARD_POINTS,34);
PSC_ACT_CALL_CC=34;
end;
...
when ( not missing(ACT_CCSS_DUEUTL) and ACT_CCSS_DUEUTL <= 0.0208333333 ) do;
...
when ( 0.21875 < ACT_CCSS_DUEUTL ) do;
...
when ( 11 < ACT_CCSS_MIN_LNINST ) do;
...
select;
when ( not missing(AGS3_MEAN_CMAXA_DUE) and AGS3_MEAN_CMAXA_DUE <= 0.3333333333 ) do;
...
when ( 1.3333333333 < AGS3_MEAN_CMAXA_DUE ) do;
SCORECARD_POINTS=sum(SCORECARD_POINTS,24);
